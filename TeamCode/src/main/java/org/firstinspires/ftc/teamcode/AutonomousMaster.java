...
public class AutonomousMaster extends LinearOpMode {
    ...
    protected static final double FRONT_WHEEL_DISTANCE = 14.8, BACK_WHEEL_DISTANCE = 14.8, FRONT_BACK_DISTANCE = 12.75, ROBOT_DIAMETER = 2 * Math.sqrt(Math.pow(1 / 2 * (FRONT_WHEEL_DISTANCE + BACK_WHEEL_DISTANCE) / 2, 2) + Math.pow(1 / 2 * FRONT_BACK_DISTANCE, 2));
    ...
    protected static final int WHEEL_DIAMETER_IN = 4, TICKS_PER_WHEEL = 1440, TICKS_PER_INCH = (int) (TICKS_PER_WHEEL / (Math.PI * WHEEL_DIAMETER_IN) + 0.5), TICKS_PER_360 = (int) (TICKS_PER_INCH * Math.PI * 10.55 + 0.5);
    ...
    protected DcMotor motorFL, motorFR, motorBL, motorBR, motorLift, motorThroat;
    protected Servo servoBucketL, servoBucketR;
    protected CRServo crServoIntakeL, crServoIntakeR;
    protected TankDrive tankDrive;
    protected Dogeforia vuforia;

    /**
     * Sets up tankDrive and vuforia, uses GoldDetection to detect and collect the gold
     */
    @Override
